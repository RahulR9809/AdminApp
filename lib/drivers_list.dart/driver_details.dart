...
class DriverDetailsPage extends StatelessWidget {
          ...
          child: Column(
            ...
            children: [
              ...
              _buildDetailRow(Icons.directions_car, "Vehicle Type",
                  vehicleDetails?['vehicle_type'] ?? 'No vehicle type'),
              _buildDetailRow(Icons.confirmation_number, "RC Number",
                  vehicleDetails?['rc_Number'] ?? 'No RC number'),
              const SizedBox(height: 16),

              // Documents Section
              _buildSectionHeader("Documents"),
              _buildImageSection(
                  context, "License Image:", driverDetails['licenseUrl']),
              _buildImageSection(
                  context, "Permit Image:", vehicleDetails?['permitUrl']),
  ...
  Widget _buildActionButtons(BuildContext context) {
    ...
    return Row(
      ...
      children: [
        if (!isAccepted) // Show the Accept button if the driver is not accepted yet
          _buildActionButton(
            context,
            'Accept',
            Colors.green,
            () {
              _showConfirmationDialog(context, 'Accept', () {
                context.read<DriverBloc>().add(
                    AcceptDriver(driverId: driverDetails['id'] ?? 'unknown'));
              });
            },
          ),
        if (isAccepted) // Show Block/Unblock buttons if the driver is accepted
          ...[
          // Block button (enabled if not blocked)
          _buildActionButton(
            context,
            'Block',
            Colors.red,
            isBlocked
                ? null // Disable if already blocked
                : () {
                    _showConfirmationDialog(context, 'Block', () {
                      context.read<DriverBloc>().add(BlockUnBlocDriver(
                          driverId: driverDetails['id'] ?? 'unknown',
                          isBlocked: true));
                    });
                  },
          ),
          // Unblock button (enabled if blocked)
          _buildActionButton(
            context,
            'Unblock',
            Colors.blue,
            !isBlocked
                ? null // Disable if not blocked
                : () {
          ...
          return AlertDialog(
            ...
            actions: [
              TextButton(
                ...
                child:
                    const Text('Cancel', style: TextStyle(color: Colors.teal)),
              ),
              ElevatedButton(
                onPressed: () {
                  onConfirm();
